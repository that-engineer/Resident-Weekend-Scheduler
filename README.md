...
A static, GitHub Pages-ready resident weekend scheduler with a React frontend, browser-local save data, and a TypeScript optimizer that runs entirely in the browser.
...
The app automatically saves editable schedule data in the browser on the same device. Refreshing or reopening the GitHub Pages site in that browser restores the last saved schedule. Use Export Backup Data to download a JSON backup, move the schedule to another device, or share editable schedule state. Use Import Saved Data to replace the current browser save with a previously exported JSON file. Use Clear Browser Save to remove the same-device browser save and start over.

Browser storage is local to the browser profile and site origin. Clearing site data, using a different browser, or using a different device will not have access to the saved schedule unless you exported a backup file.
...
Same-device browser saves use localStorage. Use the in-app Import and Export buttons to move full scheduler state as JSON files.
